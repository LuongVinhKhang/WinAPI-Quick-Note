	...
	_In_ int       nCmdShow)
{
	...
	LoadStringW(hInstance, IDC_MY1412228_QUICKNOTE, szWindowClass, MAX_LOADSTRING);
	...
	HACCEL hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDC_MY1412228_QUICKNOTE));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
	...
	wcex.lpszMenuName = MAKEINTRESOURCEW(IDC_MY1412228_QUICKNOTE);
...
BOOL InitInstance(HINSTANCE hInstance, int nCmdShow)
{
	...
	HWND hWnd = CreateWindowW(szWindowClass, szTitle, WS_OVERLAPPEDWINDOW ^ WS_THICKFRAME ^ WS_MAXIMIZEBOX,
		CW_USEDEFAULT, 0, 700, 450, nullptr, nullptr, hInstance, nullptr);
...
BOOL OnCreate(HWND hWnd, LPCREATESTRUCT lpCreateStruct)
{
	...
	CreateDirectory(L".\\Notes_Data", NULL);
	...
	listNote = CreateWindowEx(WS_EX_CLIENTEDGE, WC_LISTVIEW, _T("List View"),
		...
		240, 45, 300, 325, hWnd, (HMENU)IDC_LIST_NOTE, NULL, NULL);
	...
	// ------------------------------ Button ------------------------------
	txtNoteCount = CreateWindowEx(0, L"STATIC", TOTAL_NOTE.c_str(), WS_CHILD | WS_VISIBLE | SS_RIGHT, 390, 20, 150, 20, hWnd, NULL, hInst, NULL);

	groupTag = CreateWindowEx(0, L"BUTTON", TAG.c_str(), WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 560, 45, 100, 100, hWnd, NULL, hInst, NULL);

	btnTagRename = CreateWindowEx(0, L"BUTTON", RENAME.c_str(), WS_CHILD | WS_VISIBLE, 580, 70, 60, 25, hWnd, (HMENU)IDC_BTN_TAG_RENAME, hInst, NULL);
	btnTagDelete = CreateWindowEx(0, L"BUTTON", _DELETE.c_str(), WS_CHILD | WS_VISIBLE, 580, 105, 60, 25, hWnd, (HMENU)IDC_BTN_TAG_DELETE, hInst, NULL);

	groupNote = CreateWindowEx(0, L"BUTTON", NOTE.c_str(), WS_CHILD | WS_VISIBLE | BS_GROUPBOX, 560, 150, 100, 135, hWnd, NULL, hInst, NULL);

	btnNoteAdd = CreateWindowEx(0, L"BUTTON", ADD.c_str(), WS_CHILD | WS_VISIBLE, 580, 175, 60, 25, hWnd, (HMENU)IDC_BTN_NOTE_ADD, hInst, NULL);
	btnNoteEdit = CreateWindowEx(0, L"BUTTON", EDIT.c_str(), WS_CHILD | WS_VISIBLE, 580, 210, 60, 25, hWnd, (HMENU)IDC_BTN_NOTE_EDIT, hInst, NULL);
	btnNoteDelete = CreateWindowEx(0, L"BUTTON", _DELETE.c_str(), WS_CHILD | WS_VISIBLE, 580, 245, 60, 25, hWnd, (HMENU)IDC_BTN_NOTE_DELETE, hInst, NULL);

	btnStatistics = CreateWindowEx(0, L"BUTTON", STATISTICS.c_str(), WS_CHILD | WS_VISIBLE, 560, 300, 100, 25, hWnd, (HMENU)IDC_BTN_STATISTICS, hInst, NULL);
	...
	// myNotes.writeData();
	//myTags.addNote(L"10012017 1010", L"khang,vinh,luong");
	// myNotes.deleteNote(L"10012017 0944");

	//HWND hwnd = CreateWindowEx(0, L"STATIC", L"Loại chi tiêu :", WS_CHILD | WS_VISIBLE | SS_LEFT, 40, 50, 300, 300, hWnd, NULL, hInst, NULL);
	//SendMessage(hwnd, WM_SETFONT, WPARAM(hFont), TRUE);

	//WCHAR buffer[1024];
	//myTags.readData();
	//wsprintf(buffer, L"%s dnakjfbskjfbfkafkjbsj\n", myTags.abc());
